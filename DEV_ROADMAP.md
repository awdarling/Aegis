...
**`DOWNLOAD-500` — schedule download (Excel AND PDF) 500s on real Watermark data (PRE-EXISTING, NOT the deploy).** Status: **DIAGNOSED, fix in flight.** The throw is in **shared `buildScheduleGrid`** — the **PDF path uses zero exceljs** yet fails the same way, which proves it is not the `xlsx → exceljs` change. Prime suspect: a null/empty `employee_name` reaching `buildDisplayNameMap` (`a.employee_name.trim().split(...)` throws on null) — likely **SCHED-EDIT-1 residue** (a manual edit leaving an assignment with a missing/blank employee name). The fixture smoke test used clean names, so it never exercised this shape. Proposed fix: harden `buildScheduleGrid` (`(a.employee_name ?? '').trim()` + guard `[...(employeeDisplayNames ?? [])]`); add a route-level test with a real-shaped `ScheduleData` (null name, all-gaps, no-assignments, multi-closed); wrap the route render in `try/catch` that logs `err.stack` so the next failure isn't a blind 500. (Separately, the exceljs renderer itself was verified robust to every grid shape/content tried — empty, 0-rows/cols, multi/all-closed, control chars/emoji/surrogates — so it is not the culprit.)
...
- **Open / follow-up.** (1) Confirm the run semantic — consecutive-run-including-candidate vs. post-cap-rest-of-week lockout (the brief was ambiguous; the implementation follows the precise step-3 spec, see `BUILD_NOTES.md`). (2) Prior-week carryover if cross-week enforcement becomes important. (3) No production `policies` row exists for this constraint yet — needs a tenant configuration step before any client sees the effect.
