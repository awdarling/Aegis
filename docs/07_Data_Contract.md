...
| **D22** | ✅ **FIXED 2026-07-14 (Homebase).** `soteria_memory` is no longer written from a silent `<memory>` tag. It is now a confirmed **`save_memory` action** (executor `api/soteria/execute/route.ts`, gated by the same confirmation card + manager/owner auth as every other Soteria write), with guardrails: validated `memory_type`, non-empty content capped at 500 chars, dedupe, error-checked insert, activity log. The chat route's silent insert is deleted (defensive tag-strip kept). No engine reads this table — the fix closes the last ungated Soteria write. **Curate follow-on:** a manager-facing "What I remember" list + per-row delete in the Soteria panel (`GET/DELETE /api/soteria/memory`, manager/owner auth) makes the soft-context notes visible and prunable; the prompt now routes real schedulable preferences to policies/rules, so `soteria_memory` holds SOFT context only (never a disguised setting). Owed: one live sandbox eyeball (no Homebase test runner). |
