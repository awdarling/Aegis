...
Pipeline (see Aegis ref §2 for the file map): `buildCanvas → fill loop (buildEligibility + slot filter + rankCandidates) → resolveBannedPairConflict → enforceAttributeMixForShift (inert for Watermark) → veteran swap → evaluateSexCoverage (validate-and-flag) → gap recount`. Deterministic; no LLM. `runScheduleBuild` returns the `ScheduleData` written to `schedules.data`.
...
**Attribute mix (`enforceAttributeMixForShift`)**: post-fill swap pass enforcing per-attribute minimums (e.g. a gender minimum per shift) within `scope` (`all_shifts`/`shift_type`/`specific_shift`). Unsatisfiable → a `FlaggedIssue {type:'unsatisfied_attribute_mix'}` with a `buildAttributeShortageReason` description. Reads `employees.sex` (present in DB, absent from `src/db/types.ts`). **Inert for Watermark** since the gender rule flipped to `concurrent_coverage` (the parser yields no `attribute_mix` constraint, so this pass has nothing to act on); retained as a generic capability — fate pending (see Aegis ref §2.4 and doc 05 §6.6).

**Sex / concurrent coverage (`evaluateSexCoverage`, `src/lib/engine/sex-coverage.ts`)**: the live Watermark sex rule. Facility-wide, temporal, **validate-and-flag** — it never mutates `weekState`, so it adds no churn to hours. Algorithm, per date (restricted to assignments whose role is in `population_roles`):
- **Boundary segmentation.** Collect every shift `start_time`/`end_time` as a boundary, sort, and walk each `[t_i, t_{i+1})` block. For each block, compute the on-duty set (assignments whose window contains the block) and the set of attribute values present.
- **Single-staff skip.** Blocks with fewer than 2 on-duty are skipped — the engine never schedules single-coverage windows among counted guard roles (single staffing is reserved for Greeter/Flex, which are excluded by `population_roles`), so a 1-person block is a boundary artifact, not a real gap.
- **Missing-value detection.** For each required value with `min ≥ 1` absent from the block's present set, record the block under that missing value.
- **Contiguous-flag coalescing (second pass).** Time-contiguous segments missing the *same* value are merged into a single `unsatisfied_sex_coverage` flag (one continuous gap = one flag). A satisfied or single-staff segment between two missing blocks breaks contiguity and keeps them as separate flags. On-duty rows are deduped across coalesced segments. (Verified by a synthetic check: 3 contiguous missing segments → 1 flag 11:00–18:00; a satisfied middle window correctly stays 2 flags.)

Each flag carries `{ date, description, metadata: { time_window:{start,end}, missing_sex, on_duty:[{name,role,sex}] } }` and **no `shift_name`** (a coverage gap can straddle shifts).
...
**Rules tab** (`policies` table): key-value rules grouped by `policy_type`; the structured engine value lives in `policy_value_json`. This is where Watermark's `week_start_day='monday'` and any attribute/coverage minimums are set — Watermark's gender rule is now `sex_coverage` (`scope=concurrent_coverage`, validate-and-flag), having replaced the per-shift `attribute_mix`. (Rules tab UI work + the broader Watermark policy migration are open roadmap items — Phase 3. *Note: the Soteria system prompt still documents only the per-shift `attribute_mix` scopes; adding the `concurrent_coverage` scope there is a code change tracked separately.*)
...
- **ENGINE-1 — RESOLVED as a diagnosis (not an engine bug).** The builder was reported to silently skip an eligible employee ("Aaron Barrigan, Headguard"). Under diagnosis the two named cases collapsed to one: "Aaron Barrigan" = **Erin Berigan**, and her exclusion from every Afternoon shift was a 15-minute **availability-precision** issue (availability ended 21:00, Afternoon ends 21:15 — `isAvailableForShift` requires full containment). Data-fixed and verified. The real systemic miss is **structural, not a code bug**: 4 Junior Lifeguards (Jenna Stibitz, Cameron Osterhaven, Colin Marvin, Quin Mead) get 0h because **no `Junior Lifeguard` shift_requirements / canvas slots exist** this week — so the engine never even produces a disposition row for them. That routes to **Role Groups** (below), not an eligibility patch. Two product decisions remain pending: (a) the true Afternoon shift end-time (21:00 vs 21:15), and (b) whether Watermark schedules Junior Lifeguards at all (off-roster vs fold into Lifeguard vs Role Groups). The casing/normalization hypothesis was checked and **ruled out** for this roster (`"AManager"` is a legitimate role, not a typo).
- **`conflict_resolution_preference`** is parsed into `EngineSettings` but not yet consulted anywhere. (→ Phase 3 "rules actually apply".)
- **`doubles_policy='emergency_only'`** currently behaves like `never` (the build path has no emergency context). (→ Phase 3.)
...
- **Per-shift `attribute_mix` swap (`enforceAttributeMixForShift`) — inert, decision pending.** The Watermark gender rule is **no longer dormant**: it is **live** as `sex_coverage` (`scope=concurrent_coverage`, validate-and-flag — see §5 and Aegis ref §2.4). The old per-shift swap code remains but produces no constraint for Watermark under the flipped policy, so it never fires. Pending decision (Phase 3): keep it as a generic multi-tenant capability behind a guardrail, or remove it.
...
- **SCHED-EDIT-1 — RESOLVED (2026-06-09, Homebase `f28cb30`, live-verified).** Manual shift edits on the Homebase Schedule page previously changed the *displayed* assignment card but did not persist the corrected `shift_name`/`start_time`/`end_time` into `schedules.data.assignments`, so `distribute_schedule` sent stale (pre-move) details. The fix adds a shared pure resolver (`src/lib/schedule/resolveAssignment.ts` + `hours.ts`) called at both the move handler and the `ScheduleReviewPanel.save()` persist chokepoint: a move recomputes the full assignment (sibling-copy within the same shift_name+date; fall back to **`shift_types`** for empty targets — confirmed to match `buildCanvas`, which sources slot times from `shift_types` and role from `shift_requirements`) and recomputes hours. A manual move now round-trips corrected hours to the DB. (Live `distribute` against real data is still gated by distribution rules + DELIV-1, but the data axis is correct.)
