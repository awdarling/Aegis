...
> 7. **Homebase rules-management UI.** A page where a manager can view / create / edit / delete the veteran-experience rules per shift (reads + writes `shift_experience_rules`, mirroring the engine contract + the existing Soteria/Aegis write path). Pairs with item 3.
...
3. **Visual indicators for experience (veteran) rules.** The engine now enforces per-shift veteran rules, but nothing shows which shifts carry one. Surface a badge/indicator on the **Homebase schedule grid** AND in the **build/distribute email report** (e.g., "Veterans only" / "≥2 veterans" tag on the affected shift) so managers can see which shifts are constrained. Reads `shift_experience_rules`.
4. **Helpful privilege-denied / help message (Aegis).** Today an employee attempting a manager-only command — or anything unrecognized — gets a generic "I didn't quite understand that." Replace with a kind, role-aware reply: for a privilege block, "Sorry, that's something your manager handles," and for the fallback/`help`, a short list of the commands the sender's role CAN actually use. (Router: `MANAGER_ONLY_INTENTS` block + the `default` fallback in `intent-router.ts`.)
