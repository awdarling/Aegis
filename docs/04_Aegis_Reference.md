...
After identity verification, Claude classifies the message. Returns JSON with `intent`, `confidence` (high/medium/low), and extracted data. **EVERY date/weekday-resolving prompt must inject today's date in the TENANT's timezone** — the classifier AND every downstream extractor/generator (`extractSwapDetails`, coverage, time-off, onboarding, …). Source the timezone from `companies.timezone` (never server UTC), format it `Intl.DateTimeFormat('en-CA', { timeZone: companyTimezone })`, and hand the model the shared `weekdayAnchors` table so it never does weekday math. Server-UTC "today" and model weekday-arithmetic are both recurring day/year-drift bugs (year-drift "June 5"→wrong year; and WM-SWAP-TRADE-1, 2026-07-27: `extractSwapDetails` was on raw UTC). **Trade/pickup target-shift resolution (`chooseTradeShift`) matches on the coworker's real schedule row — its DATE and `start_time` — never on a tenant's internal shift-name strings** (employees say "her 9-3 shift on Friday", not "AM Weekday"); it resolves by day, disambiguates multiple same-day shifts by AM/PM sense, and asks "which?" rather than ever falsely reporting "no shift that week".
...
5b. **Distribution floor (FAIRNESS-2)** — `settings.fairnessFloorEnabled` (default on): a bounded, deterministic post-fill pass that rescues any eligible employee starved below `fairnessFloorRatio × role-mean` hours for the week by moving a whole slot from the most-loaded eligible holder, reusing the fill loop's eligibility primitives + a `veteranRemovalSafe` guard. Never breaks a hard rule, never double-books, never reduces coverage (reassign-only). FAIRNESS-1 cross-week memory still governs distribution above the floor. (FAIRNESS-3: that cross-week memory now imputes approved full-day time-off weeks to a normal week via the pure `foldPriorHours`, so leave is not read as under-worked — `settings.fairnessExcludeTimeOff`, default on.)
