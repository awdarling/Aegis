...
| WM-SWAP-TRADE-1 | **FIXED on branch** (2026-07-27, `fix/swap-trade-target-resolution`; IN REVIEW — pending merge + live verify) | Live Watermark: Jenna emailed *"swapping my 3-9 PM Saturday for Katie Schillaci's 9-3 PM shift on Friday"* → Aegis falsely said *"Katie doesn't have a shift that week to trade for"* though Katie worked Fri 07-31 ("AM Weekday" 11:00–15:30). **Root cause (evidence-first: DB ground-truth + Railway logs + 3× empirical extraction + deterministic replay of real `chooseTradeShift`):** the schedule was read correctly and the model extracted the target DATE correctly (`2026-07-31`), but `target_shift_name` came out as the employee's descriptor `"Friday 9-3 PM"`, and `chooseTradeShift` used it as a HARD substring filter against the internal shift NAME — `"am weekday".includes("friday 9-3 pm")` = false → 0 candidates → false `none`. Config-over-code violation (matched on a client's shift vocabulary). Even bare `"Friday"` fails. **Fix (`shift-swap.ts`):** `chooseTradeShift` resolves by the coworker's shift DATE first, uses time/name only to disambiguate MULTIPLE same-day shifts (tenant-agnostic `descriptorAmPm` off each shift's real `start_time`), and NEVER false-nones — asks "which of these?" instead. Covers trade + pickup. Plus multi-tenant hardening: `handleInitiateSwap` "today" now from `companies.timezone` (was UTC) + shared `weekdayAnchors` fed to `extractSwapDetails`. Cross-week trades OUT of scope. tsc clean; 307 pass (existing `chooseTradeShift` tests unchanged). Patch → `~/Desktop/Aegis/`. |
