...
| SCHED-EDIT-1 | **OPEN** | Manual shift edits on the Homebase Schedule page update the displayed card but do not persist corrected `shift_name`/`start_time`/`end_time` into `schedules.data.assignments`; `distribute_schedule` then sends stale hours. UI-to-data write gap, not an engine bug. |
