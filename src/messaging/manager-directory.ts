...
//
// An ABSENT key means "use the default for my role". Everyone defaults to ON.
// An owner defaults OFF for the HIGH-VOLUME operational categories only — the
// per-swap and per-publish chatter they hired managers to handle. They stay ON
// for the two things an owner actually wants: decisions that need making, and
// summaries.
//
// CORRECTED 2026-08-22. The first version of this defaulted an owner OFF for
// everything, which would have stopped a small-business owner receiving time-off
// requests the day this shipped. At Watermark that is one of two managers. An
// owner is still a manager with more authority, not less involvement — the point
// of the toggle is that they can opt OUT, not that we opt them out for them.
export type NotifyCategory = 'approvals' | 'trades' | 'schedule_posts' | 'reports';

// The categories an owner is quiet on unless they say otherwise.
const OWNER_MUTED_BY_DEFAULT: ReadonlySet<NotifyCategory> =
  new Set<NotifyCategory>(['trades', 'schedule_posts']);
 ...
 *
 * Explicit preference wins. Otherwise the role default: everyone is opted IN,
 * except an owner on the high-volume operational categories — they didn't hire
 * themselves to read every swap alert. An owner still receives approvals and
 * reports by default, because those are the two things an owner is FOR.
 *
 * A login with no person record yet gets the role default too, so linking
 * someone never silently changes what they receive.
 */
export function wantsCategory(m: ManagerContact, category: NotifyCategory): boolean {
  const explicit = m.prefs[category];
  if (typeof explicit === 'boolean') return explicit;
  if (m.role === 'owner') return !OWNER_MUTED_BY_DEFAULT.has(category);
  return true;
}
