...
| **D2** | **A swap can be `approved` while the schedule never changes.** `decision.ts` sets `swap_requests.status='approved'` *first*; `executeScheduleSwap/Trade` then silently `console.warn`+return if the week's schedule isn't found or is still **draft**. `swap_requests` has no `schedule_id`. | Employee is told the swap is approved; the published schedule still shows the old person. | Make the schedule write authoritative: write it first (or in a transaction), persist `schedule_id` on `swap_requests`, and fail loudly. |
