...
> **Push state (top-of-file):** Aegis is pushed and live (`46eaa70`). **Homebase is ahead 8 commits and UNPUSHED** — SCHED-EDIT-1 / S3 / coverage-flag display / type reconciles / CLAUDE.md secret rule are all committed-but-dark until that push lands. ENGINE-2 is resolved live in Aegis (`sex_coverage` policy flipped; manager schedule-preview email renders the flag); the Homebase schedule-view rendering of the same flag ships with the pending Homebase push.
