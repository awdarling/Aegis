...
describe('notification preferences', () => {
  it('a working manager defaults to on for everything', async () => {
  ...
  it('an owner still gets approvals and reports by default', async () => {
    // CORRECTED 2026-08-22. This used to default an owner OFF for everything,
    // which at a two-manager client like Watermark would have stopped the owner
    // receiving time-off requests the day it shipped. An owner is a manager with
    // MORE authority, not less involvement.
    const owner = mk({ role: 'owner' });
    expect(wantsCategory(owner, 'approvals')).toBe(true);
    expect(wantsCategory(owner, 'reports')).toBe(true);
  });

  it('an owner is quiet by default only on the high-volume operational chatter', async () => {
    const owner = mk({ role: 'owner' });
    expect(wantsCategory(owner, 'trades')).toBe(false);
    expect(wantsCategory(owner, 'schedule_posts')).toBe(false);
  });

  it('an owner can switch a muted category on to see what Aegis feels like', async () => {
    const owner = mk({ role: 'owner', prefs: { trades: true } });
    expect(wantsCategory(owner, 'trades')).toBe(true);
    expect(wantsCategory(owner, 'schedule_posts')).toBe(false);
  });

  it('an owner can switch approvals off — we just never do it for them', async () => {
    expect(wantsCategory(mk({ role: 'owner', prefs: { approvals: false } }), 'approvals')).toBe(false);
  });
...
describe('recipientsFor — the safety valve', () => {
  ...
  it('sends an ACTION ITEM to everyone rather than to nobody', async () => {
    // A one-owner company where the owner has explicitly switched approvals off.
    // A time-off request must never silently reach no one — no orphan outputs.
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const dir = dirOf([mk({ name: 'Ann', role: 'owner', prefs: { approvals: false } })]);
  ...
  it('an owner who has NOT opted out is a normal recipient of approvals', async () => {
    // The safety valve must not be the only reason an owner hears about a
    // time-off request — that would fire the override warning every single time.
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const dir = dirOf([mk({ name: 'Ann', role: 'owner' }), mk({ name: 'Jack' })]);
    expect(recipientsFor(dir, 'approvals', CO).map((m) => m.name)).toEqual(['Ann', 'Jack']);
    expect(warn.mock.calls.flat().join(' ')).not.toMatch(/Sending to all managers anyway/);
  });

  it('genuinely skips a non-action category nobody wants', async () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const dir = dirOf([mk({ name: 'Ann', role: 'owner', prefs: { reports: false } })]);
