...
export async function runDailySweep(): Promise<number> {
  ...
  // Bug found 2026-08-30 (carry-over check on N-4): the deactivation step used
  // to `return` early — on a load error AND on the (near-nightly, for a roster
  // this size) "nobody departed today" case — before ever reaching the two
  // housekeeping ride-alongs below. That made N-4's purge effectively
  // unreachable in production: verified live 2026-08-30, a full sweep after
  // the batch-2 deploy left all 112 pre-existing expired decision tokens and
  // 1,009 expired action tokens untouched. Housekeeping now runs in a
  // `finally` so a quiet deactivation night (or a failed load) can never skip
  // it again; each half still owns its own try/catch so one failing does not
  // stop the other.
  let flipped = 0;
  try {
    // Load the departed-but-still-active roster across all tenants. last_day is
    // a nullable date; NULL rows (no departure) are excluded by the < comparison
    // but we also guard explicitly so intent is clear.
